...
function Next() {
  ...
  const [totalEquipmentAndFacilities, setTotalEquipmentAndFacilities] =
    React.useState("");

  const [
    totalSeparatedEquipmentsAndFacilities,
    setTotalSeparatedEquipmentsAndFacilities,
  ] = React.useState("");
  ...
  React.useEffect(() => {
    queryEquipmentAmount().then((e) => {
      queryFacilityAmount().then((x) => {
        setTotalEquipmentAndFacilities(e + x + " item(s)");
        setTotalSeparatedEquipmentsAndFacilities(
          e + " Equipments & " + x + " Facilities"
        );
      });
    });
  ...
  return (
    <React.Fragment>
      <Head>
        <title>Equipment & Facility Report</title>
      </Head>
            ...
            <Grid spacing={2} container direction="row">
              <Grid item style={{ width: "33.3%" }}>
                <ReportDetailCard
                  ...
                  description={totalSeparatedEquipmentsAndFacilities}
                  title="Total Equipments & Facilities"
                  type="TotalEquipmentandFacility"
                  content={totalEquipmentAndFacilities}
                  refresh={refresh}
                />
              </Grid>
              <Grid item style={{ width: "33.3%" }}>
                <ReportDetailCard
                  refreshComponent={refreshComponent}
                  description={totalSeparatedEquipmentsAndFacilitiesReport}
                  title="Total Equipments & Facilities Report"
                  type="TotalEquipmentandFacilityReport"
                  content={totalEquipmentsAndFacilitiesReport}
                  refresh={refresh}
                />
              </Grid>
              <Grid item style={{ width: "33.3%" }}>
                <ReportDetailCard
                  refreshComponent={refreshComponent}
                  description={totalSeparatedEquipmentsAndFacilitiesRepair}
                  title="Total Equipments & Facilities Repair"
                  type="TotalEquipmentandFacilityRepair"
                  content={totalEquipmentsAndFacilitiesRepair}
                  refresh={refresh}
                />
              </Grid>
            </Grid>
