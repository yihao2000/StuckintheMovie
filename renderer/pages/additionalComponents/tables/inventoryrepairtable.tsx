...
import { Auth, InventoryRepair } from "../interfaces/interface";
...
interface EnhancedTableProps {
  ...
  onRequestSort: (
    ...
    property: keyof InventoryRepair
  ) => void;
  order: Order;
  orderBy: string;
  rowCount: number;
...
function EnhancedTableHead(props: EnhancedTableProps) {
  ...
  const createSortHandler =
    (property: keyof InventoryRepair) => (event: React.MouseEvent<unknown>) => {
...
export default function InventoryRepairTable(props: Parameter) {
  const classes = useStyles();
  const [order, setOrder] = React.useState<Order>("desc");
  const [orderBy, setOrderBy] = React.useState<keyof InventoryRepair>("id");
  ...
  const handleUpdateReport = async () => {
    if (ableToBeFixed == "yes") {
      confirmReportRepairment(selectedRepair);
    } else {
      rejectReportRepairment(selectedRepair);
    }
  ...
  React.useEffect(() => {
    ...
    queryEquipmentRepairs().then((data) => {
      data.map((e) => {
        if (props.type == "User") {
      ...
      queryFacilityRepairs().then((data) => {
        data.map((e) => {
          if (props.type == "User") {
            if (e.employeeid == auth.id) {
              array.push(
