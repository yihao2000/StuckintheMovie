...
import {
  ...
  queryEmployeePendingWarningLetterAmount,
} from "../../database/query";
...
import { Refresh } from "../interfaces/interface";
...
export function CustomizedTabs(props: ReceivedData) {
  const useStyles = makeStyles((theme: Theme) => ({
    root: {
      flexGrow: 1,
    },
...
export default function EnhancedTable(data: Refresh) {
  const classes = useStyles();
  ...
  const [pendingWarningLetterTotal, setPendingWarningLetterTotal] =
    React.useState(0);

  const [openSuccessMessage, setOpenSuccessMessage] = React.useState(false);
  const [openFailMessage, setOpenFailMessage] = React.useState(false);
  ...
  const loadEmployeeData = async (setEmployeeLoaded: any) => {
    var array = [];
    const pushData = await queryEmployees().then((data) => {
      data.map((e) => {
        array.push(
          createData(
            e.id,
  ...
  const countPendingWarningLetterTotal = (id) => {
    console.log(id);
    const result = queryEmployeePendingWarningLetterAmount(id);

    result.then((e) => {
      setPendingWarningLetterTotal(e);
    });
  };
  ...
  const handleOpenWarningLetterDialog = () => {
    if (selectedEmployee) {
      countPendingWarningLetterTotal(selectedEmployee.id);
    }
  ...
  const handleWarningLetterSubmission = () => {
    setLoading(true);
    const delay = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
    insertWarningLetter(selectedEmployee.id, selectedReason).then(async (e) => {
      await delay(2000);

      console.log(e);
      if (e == false) {
        handleOpenFailMessage();
  ...
  const handleSelectAllClick = (event: React.ChangeEvent<HTMLInputElement>) => {
    if (event.target.checked) {
      const newSelecteds = employeeList.map((n) => n.id);
        ...
        <TableContainer>
          ...
          >
            ...
            <TableBody>
              ...
              >
                <Alert onClose={handleCloseFailMessage} severity="error">
                  Failed to issue Warning Letter! Employee has maximum amount of
                  pending warning letters !
                </Alert>
              </Snackbar>
